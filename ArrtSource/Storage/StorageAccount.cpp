...
QString StorageAccount::GetSasToken(const azure::storage::storage_uri& uri, int accessTypeMask /*= azure::storage::blob_shared_access_policy::permissions::read*/, unsigned int minutes /*= 60 * 24*/) const
{
    if (m_blobClient == nullptr)
        return {};
...
QString StorageAccount::GetSasUrl(const azure::storage::storage_uri& uri, int accessTypeMask /*= azure::storage::blob_shared_access_policy::permissions::read*/, unsigned int minutes /*= 60 * 24*/) const
{
    if (m_blobClient == nullptr)
        return {};

    QString sas = GetSasToken(uri, accessTypeMask, minutes);
    ...
    std::thread([this, storageUrl = m_endpointUrl, storageCredentials = m_storageCredentials]()
                {
                    ...
                    try
                    {
                        client = std::make_unique<cloud_blob_client>(storage_uri(storageUrl.toStdWString()), storageCredentials);
                        cloud_queue_client cc(storage_uri(storageUrl.toStdWString()), storageCredentials);
...
void StorageAccount::ListBlobDirectory(const QString& containerName, const QString& prefixPath, std::vector<BlobInfo>& directories, std::vector<BlobInfo>& files) const
{
    if (m_blobClient == nullptr)
        return;
